...
- 在同样的环境文件中配置 `VITE_APP_TITLE` 来控制浏览器页签标题，未配置时默认空白。
...
- `src/services` 提供轻量 API Client 与 auth 服务，对齐共享 API 文档。

## 常见问题

- 登录提示 `not a valid bcrypt hash`：通常是后端该用户的密码 hash 不合法（seed/历史明文导致）。需要在后端重置该用户密码（重新 hash）或重新初始化认证数据库。
